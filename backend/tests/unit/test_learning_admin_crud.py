...
def test_chemical_engineering_payloads_are_valid() -> None:
    from cyberdyne_backend.application.courses.seed import ACADEMY_COURSES
    from cyberdyne_backend.cli.create_chemical_engineering_path import (
        build_payloads,
        merged_module_slugs,
    )
    from cyberdyne_backend.domain.learning import VALID_LEVELS
    ...
    assert all(m["level"] in VALID_LEVELS for m in modules)
    assert path["moduleSlugs"] == module_slugs
    assert path["slug"] == "chemical-engineering"
    catalogue = {c.slug for c in ACADEMY_COURSES}
    bundled = [s for m in modules for s in m["courseSlugs"]]
    missing = sorted(set(bundled) - catalogue)
    assert not missing, f"modules reference unseeded courses: {missing}"
...
def test_geospatial_engineering_payloads_are_valid() -> None:
    from cyberdyne_backend.application.courses.seed import ACADEMY_COURSES
    from cyberdyne_backend.cli.create_geospatial_engineering_path import (
        build_payloads,
        merged_module_slugs,
    )
    from cyberdyne_backend.domain.learning import VALID_LEVELS
    ...
    assert all(m["level"] in VALID_LEVELS for m in modules)
    assert path["moduleSlugs"] == module_slugs
    assert path["slug"] == "geospatial-engineering"
    catalogue = {c.slug for c in ACADEMY_COURSES}
    bundled = [s for m in modules for s in m["courseSlugs"]]
    missing = sorted(set(bundled) - catalogue)
    assert not missing, f"modules reference unseeded courses: {missing}"
...
def test_environmental_engineering_payloads_are_valid() -> None:
    from cyberdyne_backend.application.courses.seed import ACADEMY_COURSES
    from cyberdyne_backend.cli.create_environmental_engineering_path import (
        build_payloads,
        merged_module_slugs,
    )
    from cyberdyne_backend.domain.learning import VALID_LEVELS
    ...
    assert all(m["level"] in VALID_LEVELS for m in modules)
    assert path["moduleSlugs"] == module_slugs
    assert path["slug"] == "environmental-engineering"
    catalogue = {c.slug for c in ACADEMY_COURSES}
    bundled = [s for m in modules for s in m["courseSlugs"]]
    missing = sorted(set(bundled) - catalogue)
    assert not missing, f"modules reference unseeded courses: {missing}"
...
def test_pytorch_basics_module_payload_is_valid() -> None:
    from cyberdyne_backend.application.courses.seed import ACADEMY_COURSES
    from cyberdyne_backend.cli.add_pytorch_basics_to_computer_engineering import (
        appended_module_slugs,
        build_module_payload,
    )
    from cyberdyne_backend.domain.learning import VALID_LEVELS
    ...
    module = build_module_payload()
    assert module["slug"] == "pytorch-basics"
    assert module["level"] in VALID_LEVELS
    # The module bundles only real seeded courses.
    catalogue = {c.slug for c in ACADEMY_COURSES}
    missing = sorted(set(module["courseSlugs"]) - catalogue)
...
def test_system_design_interview_module_payload_is_valid() -> None:
    from cyberdyne_backend.application.courses.seed import ACADEMY_COURSES
    from cyberdyne_backend.cli.add_system_design_interview_to_computer_engineering import (
        appended_module_slugs,
        build_module_payload,
    )
    from cyberdyne_backend.domain.learning import VALID_LEVELS
    ...
    module = build_module_payload()
    assert module["slug"] == "system-design-interview"
    assert module["level"] in VALID_LEVELS
    catalogue = {c.slug for c in ACADEMY_COURSES}
    missing = sorted(set(module["courseSlugs"]) - catalogue)
    assert not missing, f"module references unseeded courses: {missing}"
...
def test_tensorflow_basics_module_payload_is_valid() -> None:
    ...
    module = build_module_payload()
    assert module["slug"] == "tensorflow-basics"
    assert module["level"] in VALID_LEVELS
    # The module bundles only real seeded courses.
    catalogue = {c.slug for c in ACADEMY_COURSES}
    missing = sorted(set(module["courseSlugs"]) - catalogue)
...
def test_gpu_programming_module_payload_is_valid() -> None:
    from cyberdyne_backend.application.courses.seed import ACADEMY_COURSES
    from cyberdyne_backend.cli.add_gpu_programming_to_computer_engineering import (
        appended_module_slugs,
        build_module_payload,
    )
    from cyberdyne_backend.domain.learning import VALID_LEVELS
    ...
    module = build_module_payload()
    assert module["slug"] == "gpu-programming-cuda-opencl"
    assert module["level"] in VALID_LEVELS
    catalogue = {c.slug for c in ACADEMY_COURSES}
    missing = sorted(set(module["courseSlugs"]) - catalogue)
    assert not missing, f"module references unseeded courses: {missing}"
