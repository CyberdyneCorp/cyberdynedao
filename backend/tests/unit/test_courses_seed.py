...
class TestSeedCourses:
    ...
    def test_react_basics_course_shape(self) -> None:
        course = next(c for c in ACADEMY_COURSES if c.slug == "react-basics")
        ...
        for lesson in texts[1:]:  # all but the welcome
            ...
            assert "```jsx" in body, f"{lesson.title}: missing code example"

    def test_svelte_basics_course_shape(self) -> None:
        course = next(c for c in ACADEMY_COURSES if c.slug == "svelte-basics")
        # Welcome + 8 content lessons, a checkpoint quiz after each, and the
        # comprehensive final quiz = 19.
        assert len(course.lessons) == 19
        self._assert_ai_course_pattern(course)
    ...
    def test_devops_fundamentals_course_shape(self) -> None:
        course = next(c for c in ACADEMY_COURSES if c.slug == "devops-fundamentals")
        # Welcome + 8 content lessons, a checkpoint quiz after each, and the
        # comprehensive final quiz = 19.
        assert len(course.lessons) == 19
        self._assert_ai_course_pattern(course)
        # Every content lesson carries a mermaid diagram; the technical
        # lessons carry a config/pipeline code example.
        texts = [le for le in course.lessons if le.lesson_type == "text"]
        assert len(texts) == 9
        for lesson in texts[1:]:  # all but the welcome
            assert "```mermaid" in (lesson.text_body or ""), f"{lesson.title}: missing diagram"
    ...
    def test_system_design_interview_course_shape(self) -> None:
        # A hybrid course: 9 text lessons (welcome + 8 system-design concept
        # lessons, each with a mermaid graph) and 1 freeCodeCamp video capstone
        # (with a Summary / Main ideas / Mindmap companion body), every content
        # lesson followed by a checkpoint quiz, and the final quiz last = 21.
        import re

        course = next(c for c in ACADEMY_COURSES if c.slug == "system-design-interview")
        assert len(course.lessons) == 21
        self._assert_ai_course_pattern(course)
        texts = [le for le in course.lessons if le.lesson_type == "text"]
        videos = [le for le in course.lessons if le.lesson_type == "video"]
        assert len(texts) == 9
        assert len(videos) == 1
        for lesson in texts[1:]:  # every concept lesson carries a diagram
            assert "```mermaid" in (lesson.text_body or ""), f"{lesson.title}: no diagram"
        assert all(
            v.content_url and v.content_url.startswith("https://www.youtube.com/watch?v=")
            for v in videos
    ...
    def test_curated_content_covers_all_courses(self) -> None:
        ...
        assert slugs == {
            ...
            "ai-organic-shapes-basics",
            "ai-organic-shapes-intermediate",
            "ai-organic-shapes-advanced",
            "cad-cae-parametric-basics",
            "cad-cae-parametric-intermediate",
            "cad-cae-parametric-advanced",
            "capstone-generative-mechanical-design-basics",
            "capstone-generative-mechanical-design-intermediate",
            "capstone-generative-mechanical-design-advanced",
            ...
            "generative-design-intermediate",
            "generative-design-advanced",
            "hydraulics-pneumatics-basics",
            "hydraulics-pneumatics-intermediate",
            "hydraulics-pneumatics-advanced",
            "kinematics-of-machinery-basics",
            "kinematics-of-machinery-intermediate",
            "kinematics-of-machinery-advanced",
            "machine-design-basics",
            "machine-design-intermediate",
            "machine-design-advanced",
            "mechanical-vibrations-basics",
            "mechanical-vibrations-intermediate",
            "mechanical-vibrations-advanced",
            "mechatronics-basics",
            "mechatronics-intermediate",
            "mechatronics-advanced",
            "ml-for-engineering-basics",
            "ml-for-engineering-intermediate",
            "ml-for-engineering-advanced",
            "multibody-dynamics-basics",
            "multibody-dynamics-intermediate",
            "multibody-dynamics-advanced",
            "robot-manipulators-basics",
            "robot-manipulators-intermediate",
            "robot-manipulators-advanced",
