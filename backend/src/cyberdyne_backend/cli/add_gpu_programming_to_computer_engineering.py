"""Add the "GPU Programming with CUDA and OpenCL" course to the Computer
Engineering learning path via the admin API.
...
(``seed_gpu_programming``); this script

  1. creates the ``gpu-programming-cuda-opencl`` module bundling that course
...
Safe to re-run.
    ...
    python -m cyberdyne_backend.cli.add_gpu_programming_to_computer_engineering
...
MODULE: dict[str, Any] = {
    ...
    "icon": "🖥️",
    "description": (
        "General-purpose GPU programming: the thread/block/grid model, first "
        "kernels in CUDA and OpenCL, the memory hierarchy and coalescing, a "
        "tiled matrix-multiply, optimization and pitfalls — one diagram and "
        "runnable examples per lesson."
    ),
    "topics": [
    ...
    "courseSlugs": ["gpu-programming-cuda-opencl"],
}
...
MODULE_TRANSLATIONS: dict[str, dict[str, str]] = {
    "pt-BR": {
        "title": "Programação de GPU com CUDA e OpenCL",
        "description": (
            "Programação de GPU de propósito geral: o modelo de "
            "threads/blocos/grade, primeiros kernels em CUDA e OpenCL, a "
            "hierarquia de memória e coalescência, uma multiplicação de "
            "matrizes com tiling, otimização e armadilhas — um diagrama e "
            "exemplos executáveis por lição."
        ),
    },
    "es": {
        "title": "Programación de GPU con CUDA y OpenCL",
        "description": (
            "Programación de GPU de propósito general: el modelo de "
            "hilos/bloques/rejilla, primeros kernels en CUDA y OpenCL, la "
            "jerarquía de memoria y la coalescencia, una multiplicación de "
            "matrices con tiling, optimización y errores comunes — un "
            "diagrama y ejemplos ejecutables por lección."
        ),
    },
    "fr": {
        "title": "Programmation GPU avec CUDA et OpenCL",
        "description": (
            "Programmation GPU generaliste : le modèle threads/blocs/grille, "
            "premiers noyaux en CUDA et OpenCL, la hiérarchie mémoire et la "
            "coalescence, une multiplication de matrices par tuiles, "
            "optimisation et pièges — un diagramme et des exemples "
            "exécutables par leçon."
        ),
    },
}
...
def build_module_payload() -> dict[str, Any]:
    ...
    bundles the course this feature ships (``seed_gpu_programming``)."""
