"""Add the "TensorFlow — Basics" course to the Computer Engineering
...
(``seed_tensorflow``); this script

  1. creates the ``tensorflow-basics`` module bundling that course
...
Safe to re-run.
    ...
    python -m cyberdyne_backend.cli.add_tensorflow_basics_to_computer_engineering
...
MODULE: dict[str, Any] = {
    ...
    "icon": "🧮",
    "description": (
        "Deep learning with modern TensorFlow (2.x + Keras): tensors and "
        "variables, GradientTape, Keras models, compile/fit, tf.data "
        "pipelines, custom training loops and a complete digit classifier — "
        "one diagram and runnable examples per lesson."
    ),
    "topics": [
        "Tensors & GradientTape",
        "Keras Models",
        "compile fit & tf.data",
        "Custom Training Loops",
    ],
    "courseSlugs": ["tensorflow-basics"],
}
...
MODULE_TRANSLATIONS: dict[str, dict[str, str]] = {
    "pt-BR": {
        "title": "TensorFlow — Fundamentos",
        "description": (
            "Deep learning com TensorFlow moderno (2.x + Keras): tensores e "
            "variáveis, GradientTape, modelos Keras, compile/fit, pipelines "
            "tf.data, loops de treinamento personalizados e um classificador "
            "de dígitos completo — um diagrama e exemplos executáveis por "
            "lição."
        ),
    },
    "es": {
        "title": "TensorFlow — Fundamentos",
        "description": (
            "Deep learning con TensorFlow moderno (2.x + Keras): tensores y "
            "variables, GradientTape, modelos Keras, compile/fit, pipelines "
            "tf.data, bucles de entrenamiento personalizados y un "
            "clasificador de dígitos completo — un diagrama y ejemplos "
            "ejecutables por lección."
        ),
    },
    "fr": {
        "title": "TensorFlow — Les bases",
        "description": (
            "Deep learning avec TensorFlow moderne (2.x + Keras) : tenseurs "
            "et variables, GradientTape, modèles Keras, compile/fit, "
            "pipelines tf.data, boucles d'entraînement personnalisées et un "
            "classifieur de chiffres complet — un diagramme et des exemples "
...
def build_module_payload() -> dict[str, Any]:
    ...
    bundles the course this feature ships (``seed_tensorflow``)."""
